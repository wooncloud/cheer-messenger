...
export async function getGroupPraises(groupId: string): Promise<PraiseMessage[]> {
	...
	const { data, error } = await supabase
		...
		`)
		.eq('group_id', groupId)
		.or(`is_public.eq.true,sender_id.eq.${user.user.id},receiver_id.eq.${user.user.id}`)
...
export async function canUserPraise(
	...
	if (!senderMember || !receiverMember) {
		return { canPraise: false, message: '모임 멤버만 칭찬을 주고받을 수 있습니다.' }
	}
	...
	const cooldownMs = getCooldownMs(group.praise_cooldown_value, group.praise_cooldown_unit)
...
export async function sendPraise(data: {
	...
	const praiseCheck = await canUserPraise(data.groupId, user.user.id, data.receiverId)
...
function getCooldownMs(value: number, unit: string): number {
	...
	return value * (multipliers[unit as keyof typeof multipliers] || multipliers.day)
}
