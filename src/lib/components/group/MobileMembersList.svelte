					...
					<div class="flex-1">
						...
						{#if member.recent_praises && member.recent_praises.length > 0}
							<div class="flex flex-wrap gap-1 max-w-xs">
								{#each member.recent_praises.slice(0, 6) as praise}
									<div 
									...
									>
										...
										{#if praise.message}
											<span class="ml-1 text-gray-600 max-w-[50px] truncate">{praise.message}</span>
										{/if}
									</div>
								{/each}
								{#if member.recent_praises.length > 6}
									<span class="text-xs text-gray-500 self-center">+{member.recent_praises.length - 6}개</span>
								{/if}
							</div>
						{:else}
