<script lang="ts">
  ...
  async function handleSendPraise() {
    ...
    try {
      ...
      toast.success(
        `${selectedMember.user.name}님에게 칭찬을 보냈습니다! ${praiseForm.emoji}`
      );
      closePraiseModal();
...
<div class="container mx-auto py-6">
  {#if !groupDataLoaded || !data.group}
    ...
    <div class="text-center py-12">
      ...
      ></div>
      <p class="text-muted-foreground">그룹 정보를 불러오는 중...</p>
    </div>
  {:else}
    <!-- Header -->
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div class="text-xs text-muted-foreground">
                          ...
                          {#if !praise.is_public}
                            <span
                              class="ml-2 text-xs bg-muted px-2 py-1 rounded"
                              >비공개</span
                            >
                          {/if}
                        </div>
    ...
    >
      <div class="bg-background rounded-lg p-6 w-full max-w-md">
        ...
        <div class="space-y-4">
          ...
          {#if praiseError}
            <div
              class="text-destructive text-sm bg-destructive/10 p-3 rounded-md"
            >
              {praiseError}
            </div>
          {/if}
